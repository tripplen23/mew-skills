...
2. **Build `semantic_map`.** Include only risk categories that occur in the scoped units. Allowed categories are `ownership`, `nullable`, `exceptions`, `integer_overflow`, `time`, `string_encoding`, `concurrency`, `allocator`, `ffi`, `debug_release`, and `platform`. Do not add empty or speculative entries to cover the list. Each entry records `source_pattern`, `target_pattern`, `risk`, and applicable `gotchas`. For Python→Rust ports, load `references/python-rust.md` and copy its gotchas into the entries where they apply (timestamp format pinning → `time`; axum route syntax and problem+json / HTTP status shapes → `exceptions`; POST-204 / DELETE-200 actuals → `exceptions` or `ownership`). If a gotcha has no fitting category (e.g. framework route syntax), record it in `exceptions` with a `gotchas` note naming the framework, and reference the contract property it protects in the owning unit — the schema does not define http/routing categories, so do not invent new risk values.
